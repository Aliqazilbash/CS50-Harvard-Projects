...
void tabulate(void)
{
    ...
    // Add votes to non-eliminated top-ranked candidates
    for (int i = 0; i < voter_count; i++)
    {
        for (int j = 0; j < candidate_count; j++)
        {
...
bool print_winner(void)
{
    // Check if any candidate holds the majority of the votes
...
int find_min(void)
{
    // Set min votes to the first candidate's votes
    int min_votes = candidates[0].votes;

    // If any candidate has fewer votes, update min votes
    for (int i = 0; i < candidate_count; i++)
    {
...
bool is_tie(int min)
{
    // Check if every candidate has the same number of votes as min
    for (int i = 0; i < candidate_count; i++)
    {
